...
void max30102_init()
{
    ...
    /*New data interrupted*/
    data = 0x40;
    HAL_I2C_Mem_Write(&hi2c2, MAX30102_ADDR_WRITE, RES_INTERRUPT_ENABLE_1, I2C_MEMADD_SIZE_8BIT, &data, 1, 10);
    //    /* Interrupt after temperature conversion*/
    //    data = 0x02;
    //    HAL_I2C_Mem_Write(&hi2c2, MAX30102_ADDR_WRITE, RES_INTERRUPT_ENABLE_2, I2C_MEMADD_SIZE_8BIT, &data, 1, 10);
    //    /*25 Sample unread interrupt*/
    //    data = 0x7;
    //    /* Almost full interrupt*/
    //    data = 0x80;
    //    HAL_I2C_Mem_Write(&hi2c2, MAX30102_ADDR_WRITE, RES_INTERRUPT_ENABLE_1, I2C_MEMADD_SIZE_8BIT, &data, 1, 10);
    //    HAL_I2C_Mem_Write(&hi2c2, MAX30102_ADDR_WRITE, RES_FIFO_CONFIGURATION, I2C_MEMADD_SIZE_8BIT, &data, 1, 10);
    /*16384Range50Hz 18Bit adc resolution*/
    data = 0x63;
    ...
    data = 0x47;
    HAL_I2C_Mem_Write(&hi2c2, MAX30102_ADDR_WRITE, RES_LED_PLUSE_AMPLITUDE_1, I2C_MEMADD_SIZE_8BIT, &data, 1, 10);
    HAL_I2C_Mem_Write(&hi2c2, MAX30102_ADDR_WRITE, RES_LED_PLUSE_AMPLITUDE_2, I2C_MEMADD_SIZE_8BIT, &data, 1, 10);
    HAL_I2C_Mem_Write(&hi2c2, MAX30102_ADDR_WRITE, RES_PROXIMITY_MODE_LED_PLUSE_AMPLITUDE, I2C_MEMADD_SIZE_8BIT, &data, 1, 10);
    /*FIFO clear*/
    data = 0;
    ...
    // data = 1;
    // HAL_I2C_Mem_Write(&hi2c2, MAX30102_ADDR_WRITE, RES_DIE_TEMPERATURE_CONFIG, I2C_MEMADD_SIZE_8BIT, &data, 1, 10);
    /*SPO2 Mode*/
    data = 0x03;
    HAL_I2C_Mem_Write(&hi2c2, MAX30102_ADDR_WRITE, RES_MODE_CONFIGURATION, I2C_MEMADD_SIZE_8BIT, &data, 1, 10);
}
...
uint8_t max30102_getStatus()
{
    ...
    HAL_I2C_Mem_Read(&hi2c2, MAX30102_ADDR_READ, RES_INTERRUPT_STATUS_1, I2C_MEMADD_SIZE_8BIT, &dataTemp, 1, 10);
    data = dataTemp;
    HAL_I2C_Mem_Read(&hi2c2, MAX30102_ADDR_READ, RES_INTERRUPT_STATUS_2, I2C_MEMADD_SIZE_8BIT, &dataTemp, 1, 10);
    return data | dataTemp;
}

// Use this to read state to determine whether the conversion is complete
// float max30102_getTemperature()
// {
//     int8_t dataTemp = 0;
//     float data;
//     HAL_I2C_Mem_Read(&hi2c2, MAX30102_ADDR_READ, RES_DIE_TEMP_INTEGER, I2C_MEMADD_SIZE_8BIT, (uint8_t *)(&dataTemp), 1, 10);
//     data = dataTemp;
//     HAL_I2C_Mem_Read(&hi2c2, MAX30102_ADDR_READ, RES_DIE_TEMP_FRACTION, I2C_MEMADD_SIZE_8BIT, (uint8_t *)(&dataTemp), 1, 10);
//     data += dataTemp * 0.0625;
//     // Start the next conversion
//     dataTemp = 1;
//     HAL_I2C_Mem_Write(&hi2c2, MAX30102_ADDR_WRITE, RES_DIE_TEMPERATURE_CONFIG, I2C_MEMADD_SIZE_8BIT, (uint8_t *)(&dataTemp), 1, 10);
//     return data;
// }
...
#define FILTER_LEVEL 8 /*Filter level*/
...
int16_t eachSampleDiff = 0; //How far is it from the previous sample
void max30102_cal()
{
    ...
    static uint8_t eachBeatSampleCount = 0;    //How many samples have passed through this heartbeat
    static uint8_t lastTenBeatSampleCount[10]; //The number of samples for each of the past ten heartbeats
    static uint32_t last_iRed = 0;             //Last infrared value, filtered
    uint8_t i, ii;
    for (i = 0; i < unreadSampleCount; i++)
    {
        if (sampleBuffTemp[i].iRed < 40000) //No fingers do not count, skip
        {
            heartRate = 0;
            spo2 = 0;
            eachSampleDiff = 0;
        ...
        //Calculate heart rate,30-250ppm  count:200-12
